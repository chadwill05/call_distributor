...
def update_caller_listbox(caller):
    caller_listboxes[caller].delete(0, tk.END)  # Clear existing contacts
    for contact in caller_call_lists[caller]:
        index = caller_listboxes[caller].insert(tk.END, f"{contact[0]} - {contact[1]}")
        caller_listboxes[caller].itemconfig(index, {'bg': 'white'})  # New calls will have white background
...
def create_main_window():
    ...
    # Creating a frame for each caller
